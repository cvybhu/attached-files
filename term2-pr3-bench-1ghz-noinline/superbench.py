...
def build_bench_binaries(branch_commits):
    run("./configure.py --mode release --clang-inline-threshold 16")
...
def build_flame_binaries(branch_commits):
    run("./configure.py --mode release --cflags=-fno-omit-frame-pointer --clang-inline-threshold 16")
    for commit in branch_commits:
        run(f"git checkout {commit}")
...
def run_single_benchmark(commit, workload_name, master_plus, i):
    ...
    bench_out = run(f"{workdir}/bin/bench/perf_simple_query_bench_{commit} --cpuset 1 -m 1G --task-quota-ms 10 --random-seed 0 --operations-per-shard {ops}")
...
def run_flame_bench(commit, workload_name, master_plus):
    ...
    start_time = time.time()
    bench_out = run(f"perf record -F 99 -g -o {perf_data_path} -- {bin_path} --cpuset 1 -m 1G --task-quota-ms 10 --random-seed 0 --operations-per-shard {ops}")
    run_time = time.time() - start_time
